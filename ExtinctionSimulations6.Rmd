...
# Scenario 1 
(ex_extinct_t <- second.extinct(bipart_0s_nothreat_t, participant="lower", method="random", nrep=545, 
                              details=F))
ex_extinct_t
...
# Scenario 2
(ex2_extinct_t <- second.extinct(bipart_0s_nothreat_t, participant="lower", method="degree", nrep=545, 
                               details=F))
ex2_extinct_t
summary(ex2_extinct_t)

rob_ext_2_t<-robustness(ex2_extinct_t)
...
#STRUCTURE PLOTS
 ...
 write.csv(structure_data_random, "structure_data_random.csv")
modularity_data <- read.csv("structure_data_random.csv")
...
```{r Specialization}
#Specialization for the combined network
bipart_t <- t(bipart)
combined_specialization <- dfun(bipart)
combined_specialization_l <- as.data.frame(combined_specialization$d)
combined_specialization_l$Species <- rownames(combined_specialization_l)
combined_specialization_p <- dfun(bipart_t)
combined_specialization_p <- as.data.frame(combined_specialization_p$d)
combined_specialization_p$Species <- rownames(combined_specialization_p)
combined_special_h <- H2fun(bipart)

#Specialization for the mutualisitc network
bipart_frugivore_t <- t(bipart_frugivore)
mut_specialization <- dfun(bipart_frugivore)
mut_specialization_l <- as.data.frame(mut_specialization$d)
mut_specialization_l$Species <- rownames(mut_specialization_l)
mut_specialization_p <- dfun(bipart_frugivore_t)
mut_specialization_p <- as.data.frame(mut_specialization_p$d)
mut_specialization_p$Species <- rownames(mut_specialization_p)
mut_special_h <- H2fun(bipart_frugivore)

#Specialization for the antagonistic relationship
bipart_antagonistic_t <- t(bipart_antagonistic2)
ant_specialization <- dfun(bipart_antagonistic2)
ant_specialization_l <- as.data.frame(ant_specialization$d)
ant_specialization_l$Species <- rownames(ant_specialization_l)
ant_specialization_p <- dfun(bipart_antagonistic_t)
ant_specialization_p <- as.data.frame(ant_specialization_p$d)
ant_specialization_p$Species <- rownames(ant_specialization_p)
ant_special_h <- H2fun(bipart_antagonistic2)

#Specialization for the combined network post-extinction
bipart_t_extinct <- t(bipart_0s_nothreat)
combined_specialization_extinct  <- dfun(bipart_0s_nothreat)
combined_specialization_l_extinct  <- as.data.frame(combined_specialization_extinct$d)
combined_specialization_l_extinct$Species <- rownames(combined_specialization_l_extinct)
combined_specialization_p_extinct  <- dfun(bipart_t_extinct)
combined_specialization_p_extinct <- as.data.frame(combined_specialization_p_extinct$d)
combined_specialization_p_extinct$Species <- rownames(combined_specialization_p_extinct)
combined_special_h_extinct <- H2fun(bipart_0s_nothreat)

#Specialization for the mutualisitc network post-extinction
bipart_frugivore_t_extinct <- t(bipart_0s_nothreat_fruit)
mut_specialization_extinct <- dfun(bipart_0s_nothreat_fruit)
mut_specialization_l_extinct <- as.data.frame(mut_specialization_extinct$d)
mut_specialization_l_extinct$Species <- rownames(mut_specialization_l_extinct)
mut_specialization_p_extinct <- dfun(bipart_frugivore_t_extinct)
mut_specialization_p_extinct <- as.data.frame(mut_specialization_p_extinct$d)
mut_specialization_p_extinct$Species <- rownames(mut_specialization_p_extinct)
mut_special_h_extinct <- H2fun(bipart_0s_nothreat_fruit)

#Specialization for the antagonistic relationship post-extinction
bipart_antagonistic_t_extinct <- t(bipart_0s_nothreat_ant)
ant_specialization_extinct <- dfun(bipart_0s_nothreat_ant)
ant_specialization_l_extinct <- as.data.frame(ant_specialization_extinct$d)
ant_specialization_l_extinct$Species <- rownames(ant_specialization_l_extinct)
ant_specialization_p_extinct <- dfun(bipart_antagonistic_t_extinct)
ant_specialization_p_extinct <- as.data.frame(ant_specialization_p_extinct$d)
ant_specialization_p_extinct$Species <- rownames(ant_specialization_p_extinct)
ant_special_h_extinct <- H2fun(bipart_0s_nothreat_ant)

#Combine lemur specialization data
lemur_specialization <- left_join(combined_specialization_l, mut_specialization_l, by="Species")
lemur_specialization <- left_join(lemur_specialization, ant_specialization_l, by="Species")
lemur_specialization <- left_join(lemur_specialization,combined_specialization_l_extinct, by="Species")
lemur_specialization <- left_join(lemur_specialization,mut_specialization_l_extinct, by="Species")
lemur_specialization <- left_join(lemur_specialization,ant_specialization_l_extinct, by="Species")

colnames(lemur_specialization) <- c("Combined", "Species", "Mutualistic", "Antagonistic", "Combined Post-Extinction", "Mutualistic Post-Extinction", "Antagonistic Post-Extinction")
lemur_specialization2 <- lemur_specialization %>%
  select(Species, Combined, Mutualistic, Antagonistic, `Combined Post-Extinction`, `Mutualistic Post-Extinction`, `Antagonistic Post-Extinction`)

mean(lemur_specialization2$Combined)
mean(lemur_specialization2$Mutualistic, na.rm = T)
mean(lemur_specialization2$Antagonistic, na.rm = T)
mean(lemur_specialization2$`Combined Post-Extinction`, na.rm = T)
mean(lemur_specialization2$`Mutualistic Post-Extinction`, na.rm = T)
mean(lemur_specialization2$`Antagonistic Post-Extinction`, na.rm = T)

#combine plant specialization data 
plant_specialization <- left_join(combined_specialization_p, mut_specialization_p, by="Species")
plant_specialization <- left_join(plant_specialization, ant_specialization_p, by="Species")
plant_specialization <- left_join(plant_specialization,combined_specialization_p_extinct, by="Species")
plant_specialization <- left_join(plant_specialization,mut_specialization_p_extinct, by="Species")
plant_specialization <- left_join(plant_specialization,ant_specialization_p_extinct, by="Species")
colnames(plant_specialization) <- c("Combined", "Species", "Mutualistic", "Antagonistic", "Combined Post-Extinction", "Mutualistic Post-Extinction", "Antagonistic Post-Extinction")
plant_specialization2 <- plant_specialization %>%
  select(Species, Combined, Mutualistic, Antagonistic, `Combined Post-Extinction`, `Mutualistic Post-Extinction`, `Antagonistic Post-Extinction`)

mean(plant_specialization2$Combined)
mean(plant_specialization2$Mutualistic, na.rm = T)
mean(plant_specialization2$Antagonistic, na.rm = T)
mean(plant_specialization2$`Combined Post-Extinction`, na.rm = T)
mean(plant_specialization2$`Mutualistic Post-Extinction`, na.rm = T)
mean(plant_specialization2$`Antagonistic Post-Extinction`, na.rm = T)

plant_specialization2$Combined <- round(plant_specialization2$Combined,3)
plant_specialization2$Mutualistic<- round(plant_specialization2$Mutualistic,3)
plant_specialization2$Antagonistic<- round(plant_specialization2$Antagonistic,3)
plant_specialization2$`Combined Post-Extinction` <- round(plant_specialization2$`Combined Post-Extinction`,3)
plant_specialization2$`Mutualistic Post-Extinction`<- round(plant_specialization2$`Mutualistic Post-Extinction`,3)
plant_specialization2$`Antagonistic Post-Extinction`<- round(plant_specialization2$`Antagonistic Post-Extinction`,3)

lemur_specialization2$Combined <- round(lemur_specialization2$Combined,3)
lemur_specialization2$Mutualistic<- round(lemur_specialization2$Mutualistic,3)
lemur_specialization2$Antagonistic<- round(lemur_specialization2$Antagonistic,3)
lemur_specialization2$`Combined Post-Extinction` <- round(lemur_specialization2$`Combined Post-Extinction`,3)
lemur_specialization2$`Mutualistic Post-Extinction`<- round(lemur_specialization2$`Mutualistic Post-Extinction`,3)
lemur_specialization2$`Antagonistic Post-Extinction`<- round(lemur_specialization2$`Antagonistic Post-Extinction`,3)

lemur_specialization2$Species  <- gsub("_", " ", lemur_specialization2$Species)

write.csv(plant_specialization2, "plant_specialization.csv")
write.csv(lemur_specialization2, "lemur_specialization.csv")
